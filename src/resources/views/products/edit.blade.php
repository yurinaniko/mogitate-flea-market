    ...
    <div class="product-edit__title">
        ...
        <div class="product-edit__wrapper">
            ...
            <div class="product-edit__left">
                ...
                <label class="product-edit__file-label">
                    ファイルを選択
                    <input type="file" name="image" id="image" class="product-edit__file" accept=".png, .jpeg, .jpg" onchange="previewImage(event)">
                </label>
            </div>
            {{-- 右カラム：テキスト情報 --}}
        ...
        <div class="form-row description-row">
            <label class="form-label">商品説明</label>
                <textarea name="description" class="form-textarea" rows="6" placeholder="商品の説明を入力してください">{{ old('description', $product->description ?? '') }}</textarea>
                @error('description')
                    <p class="form__error">{{ $message }}</p>
                @enderror
        </div>
